// 👋 Dependent Drop-Down Data Validation Lists. DVL-2021.
...
//    #source_other_file
// 
//  Will work if you install onEdit trigger and
//  give it permissions
...
// ♟️ Sample settings ↓  
/***************************************************** 
  // [#0] sample of all possible sets,
  // some of them may be skipped
    {                     
      dict: "'Dictionary'!A2:E",
          // range with data, may use open range, 
          // tip: use sheet name in single quotes ''
          // * Required    
          //
      sheet: "Dropdowns",  
          // work sheet name
          // where to make validation
          // * Required
          //
      first_row: 2,
          // row to start data validation
          // Default: 2            
          //
      first_column: 'A',
          // column to start datavalidation - level 1
          // Default: 'A' 
          //
      last_row: 100500,  
          // last row of validation, infinite, if missing
          // Default: ∞ (infinity)
          //
      columns: ['A', 'B', 'D'], 
          // columns with validation levels, 
          // first_column is ignored if set   
          // Default: null              
          //
      last_is_range: false,
          // use last level as a range reference 
          // for data validation,
          // tip: set last_is_range =  true and 
          //      use formulas like: ='MySheet'!A1:A1500"
          // Default: false             
          //
...
function getTestEdit_DVL_() {
  ...
  var e = {
    test: true,
    source: f,
    range: f.getRange("Dropdowns!M4"),  // 'Dropdowns+'!A6:A26
...
function dvl_row_(set, Tasker, Dicts) {
  ...
  var dict;
  if (!last_user_value || last_user_value === '') {
